...
double Prob(double L0, double L1, double T){
	double p;
	if(L1<=L0) p = 1;
	else{
		p=exp(-(L1-L0)/T);
...
SA::SA(double Tstart, int NTsteps, int Nsubsteps, int Nc, int seed_external){
	NTS = NTsteps;
	NSS = Nsubsteps;
	sT=Tstart;
	cT=sT;
	...
	Ng = Nc;
	dT=double(sT/NTS);
	for(int i = 0; i <Ng; i++) config.push_back(i);
	for(int i = 0; i <Ng; i++) config1.push_back(i);
	for(int i = 0; i <Ng; i++) bst_config.push_back(i);
	random_shuffle(config.begin(), config.end());
	cout << "starting temperature: " << sT << endl;
	cout << "number of temperature steps: " << NTS << endl;
	cout << "number of steps for each temperature: " << NSS << endl;
	cities.resize(Ng);
	int seed[4];
  int p1, p2;
...
void SA::GenerateCities(int mode){
	if(mode==0){
		for(int i = 0; i < Ng; i++){
...
double SA::Run(int num){
	string fname = "best_path_temp";
	fname+=to_string(num);
	fname+=".dat";
	ofstream fout(fname);
	double L0,L1;
	for(is=0;is<NTS;is++){
		cout << "current temperature: " << cT << endl;
		cout << "temperature step: " << is << "/" << NTS << endl;
		int N=0;
		int Nacc=0;
		for(icT=0; icT < NSS; icT++){
			config1=config;
			for(int j=0; j<6; j++) if(rnd.Rannyu()<=1.) Mutate(j);
			tie(L0,L1) = evaluate();
			N++;
			if(L1<L_bst){
			bst_config=config1;
	...
	cout << "accettazione: " << float(Nacc)/N << endl;
	fout << is << "     " << L_bst << endl;
	cT-=dT;
	}
	fout.close();
	cout <<"saving best individual" <<endl;
	SaveBest(2);
	return L_bst;
}
...
pair<double,double> SA::evaluate(){
	double x = 0, y =0;
	for(int i = 0; i < config.size()-1; i++) x+=distance(cities[config[i]][0],cities[config[i]][1],cities[config[i+1]][0],cities[config[i+1]][1]);
	for(int i = 0; i < config1.size()-1; i++) y+=distance(cities[config1[i]][0],cities[config1[i]][1],cities[config1[i+1]][0],cities[config1[i+1]][1]);
	return make_pair(x,y);
}
void SA::Mutate(int m){ // m = type of mutation. i = individual to mutate
	if(m==0){ //permutate 2 genes
		int times=int(rnd.Rannyu()*10.)+1;
		for(int z=0; z<times; z++){
		int a = int(Ng*rnd.Rannyu());
		int b = GenPBC(int(Ng*rnd.Rannyu())+2);
		int t =	config1[a];
		config1[a]=config1[b];
		config1[b] = t;
	}}
		...
		rotate(config1.begin(), config1.begin()+s, config1.end());			
	}}
		...
		int bk[d];
		for(int k = 0; k < d; k++){
			bk[k]=config1[GenPBC(b+k)];
		}
		for(int k = 0; k<s; k++){
			config1[GenPBC(b+k)]=config1[GenPBC(b+d+k)];
		}
		for(int k=0; k < d; k++){
			config1[GenPBC(b+s+k)]=bk[k];
		}
	}
		...
		int buff;
		for(int k = 0; k < d; k++){
			buff=config1[GenPBC(a+k)];
			config1[GenPBC(a+k)]=config1[GenPBC(b+k)];
			config1[GenPBC(b+k)]=buff;
		}	
	}
		...
		int bk[d];
		for(int k = 0; k < d; k++){
			bk[k]=config1[GenPBC(a+k)];
		}
		for(int k = 0; k < d; k++){
			config1[GenPBC(a+d-k)]=bk[k];
		}
	}
	else if(m==5){
	if(rnd.Rannyu() < 0.2){
	random_shuffle(config1.begin(), config1.end());
	}}
	else{
...
void SA::SaveBest(int n){
	string fname="best_individual";
	fname += to_string(n);
	fname += ".dat";
	ofstream best(fname);
	for(int i=0; i<bst_config.size(); i++){ //export cities positions
		best << cities[bst_config[i]][0] << " " << cities[bst_config[i]][1] << endl;
	}
	best.close();
}

void SA::LoadCities(string fname){
	ifstream read(fname);
	double x, y;
	if (read.is_open()) {
	for(int j =0; j<Ng; j++) {
	cities[j] = vector<double>(2);
	read >> x >> y;
	cities[j][0]=x;
	cities[j][1]=y;
}
