...
int main(int argc, char* argv[]){
	...
	if(size!=4){cout<<"Eseguire 4 processi"<<endl;
	return 1;}
	double irecv[4];
	for(int i=0;i<3;i++) irecv[i]=0;
	SA sa((0.8), 1000, 1000, 30, rank*37);
	cout << "inizializzato SA (Simulated Annealing)" << endl;
	//sa.GenerateCities(0);
	sa.LoadCities("circ_cities"+to_string(rank)+".dat");
	double isend = sa.Run(rank);
	MPI_Gather(&isend,1,MPI_DOUBLE_PRECISION,irecv,1,MPI_DOUBLE_PRECISION,0,
	MPI::COMM_WORLD);
	if(rank==0) cout<< "irecv: " <<irecv[0] <<" "
	<<irecv[1] <<" " <<irecv[2] << " " << irecv[3]<< endl;
	MPI::Finalize();
	return 0;
