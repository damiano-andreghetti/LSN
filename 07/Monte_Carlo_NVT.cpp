...
// contribution to energy and virial
  ...
  walker[iw] = 48.0 * w / 3.0;
	//export U/N and P istantaneous
	/*
	double P = rho * temp + (walker[iw]+ ptail * (double)npart) / vol;
	double UN = walker[iv]/(double)npart + vtail;
	ofstream insta;
	insta.open("insta.dat",ios::app);
	insta << UN << " " << P <<endl;
	insta.close();
	*/
}
...
void Gdirave(void){
	...
	for(int i = 0; i<nbins; i++){
		...
		Gave << setw(wd) << r << setw(wd) << glob_av[igofr+i]/(double)nblk << setw(wd) << err << endl;
	}
	Gave.close();
}
/****************************************************************
