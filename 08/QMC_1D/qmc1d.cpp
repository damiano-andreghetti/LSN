...
double variationalWaveFunction(double v)
{
	double mu=0.8;
	double sig=0.8;
	return exp(-(v-mu)*(v-mu)/(2*sig*sig))+exp(-(v+mu)*(v+mu)/(2*sig*sig));
	//return exp(-0.5*v*v);
}
...
double variationalWaveFunction_second(double v)
{
	double mu=0.8;
	double sig=0.8;
	return (-1/(sig*sig))*((exp(-(v-mu)*(v-mu)/(2*sig*sig))*(1-((v-mu)/sig)*((v-mu)/sig)))+((exp(-(v+mu)*(v+mu)/(2*sig*sig))*(1-((v+mu)/sig)*((v+mu)/sig)))));
